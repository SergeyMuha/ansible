#! /bin/bash
if [ -z "$1" ]; then 
	echo usage: boot_aws_vm.sh amis
	echo amis : ami-38708b45 - ubuntu 14.04, ami-a0d009dd - ubuntu 16.04, ami-467ca739 - amazon
	exit
fi

id=$(aws ec2 run-instances --image-id $1 --key-name aws_console_key --instance-type t2.micro --tag-specifications "ResourceType=instance,Tags=[{Key=OStype,Value=$1}]" | grep -i InstanceId  | awk -F':' '{print $2}' | sed 's/"//g' | sed 's/,//g' | sed 's/ //g')
sleep 10
